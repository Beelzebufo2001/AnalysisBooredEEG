...
def run_subject(subject, subject_dir):
    ...
    for label, (idxA, idxB) in { # Shitty for loop BUT basic usage
        "left <-> left":   (left,  left),
        "left <-> right":  (left,  right),
        "right <-> right": (right, right),
    }.items(): # ITEMISE list >>> I dont like it this short
        mean, std = region_connectivity(connectivity, idxA, idxB) # Compute conectivity
        save_mean_std_plot(mean, std, label, subject, out_dir) # Save that b >> TODOO corect

    # ── brain regions ──────────────────────────────────────────────────────────
    regions = build_region_indices(ch_names)

    for nameA, regA in regions.items():#All mit All
